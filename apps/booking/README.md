...
- 每堂課程、DUPR 場都有自己的分享網址 `https://<網域>/e/<8 碼代碼>`，分享到 LINE／FB 會顯示活動名稱、時間、地點、報名人數與場館封面。
- 打開後是一頁式報名頁：沒有場館導覽，只有活動資訊、主辦單位、報名與登入（登入或 LINE 登入後會回到同一頁）。
- 建課時勾「只限連結報名」就是一次性活動：不出現在課表與老師頁，只有拿到連結的人看得到、報得了名（代碼不可猜）；已報名的人仍可從會員中心點進去。
- 後台照常管理（名單、點名、DUPR 賽事）；名單頁有「報名連結」可複製、分享到 LINE、預覽報名頁。
